...
function resolveClientConfig(platform: string | undefined): OAuthClientConfig {
  ...
  if (platform === 'ios') {
    const clientId = env.VITE_GOOGLE_IOS_CLIENT_ID;
    if (!clientId) {
    ...
    return {
      clientId,
      scheme:
        env.VITE_GOOGLE_OAUTH_REDIRECT_SCHEME ??
        reversedClientIdScheme(clientId),
      clientSecret: env.VITE_GOOGLE_IOS_CLIENT_SECRET,
    };
  }
  ...
  const schemeOverride =
    platform === 'android'
      ? env.VITE_GOOGLE_OAUTH_REDIRECT_SCHEME_ANDROID
      : env.VITE_GOOGLE_OAUTH_REDIRECT_SCHEME_DESKTOP;
  return {
    clientId,
