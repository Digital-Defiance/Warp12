import type { FC } from 'react';

export interface Warp12LogoProps {
  width?: number;
  className?: string;
  warpColor?: string;
  numberColor?: string;
  taglineColor?: string;
  taglineOff?: boolean;
  marginLeft?: string;
  factor?: number;
}

const GALAXY_FAINT_PATH =
  'M64 244.5L64 256C64 326.3 101.8 387.8 158.2 421.2C186.9 438.2 220.3 448 256 448C278.3 448 299.7 444.2 319.5 437.2C311.1 463.1 296.5 486.7 277.1 506C267.9 515.1 257.5 523.3 246.2 530.3L233.8 538C227.7 541.7 224 548.4 224 555.5C224 566.8 233.2 576 244.5 576L256 576C326.3 576 387.8 538.2 421.2 481.8C438.2 453.1 448 419.7 448 384C448 361.7 444.2 340.3 437.2 320.5C463.1 328.9 486.7 343.5 506 362.9C515.1 372.1 523.3 382.5 530.3 393.8L538 406.3C541.7 412.4 548.4 416.1 555.5 416.1C566.8 416.1 576 406.9 576 395.6L576 384.1C576 313.8 538.2 252.3 481.8 218.9C453.1 201.9 419.7 192.1 384 192.1C361.7 192.1 340.3 195.9 320.5 202.9C328.9 177 343.5 153.4 362.9 134.1C372.1 125 382.5 116.8 393.8 109.8L406.2 102C412.3 98.3 416 91.6 416 84.5C416 73.2 406.8 64 395.5 64L384 64C313.7 64 252.2 101.8 218.8 158.2C210.3 172.5 203.6 188.1 199 204.5C196.7 212.7 195 221.1 193.8 229.7C193.2 234 192.8 238.4 192.5 242.7C192.2 247 192 251.5 192 256C192 278.3 195.8 299.7 202.8 319.5C176.9 311.1 153.3 296.5 134 277.1C124.9 267.9 116.7 257.5 109.7 246.2L102 233.8C98.3 227.7 91.7 224 84.5 224C73.2 224 64 233.2 64 244.5zM352 320C352 337.7 337.7 352 320 352C302.3 352 288 337.7 288 320C288 302.3 302.3 288 320 288C337.7 288 352 302.3 352 320z';

const GALAXY_SOLID_PATH =
  'M218.8 158.2C218.9 145.2 220.4 132.1 223.5 119.2L226.9 105C228.6 98.1 226.5 90.8 221.4 85.7C213.4 77.7 200.4 77.7 192.4 85.7L184.3 93.8C134.6 143.5 117.8 213.7 134 277.2C153.3 296.6 176.9 311.2 202.8 319.6C195.8 299.7 192 278.3 192 256.1C192 220.4 201.8 186.9 218.8 158.3zM158.2 421.2C145.2 421.1 132.1 419.6 119.2 416.5L105 413.1C98.1 411.4 90.8 413.5 85.7 418.6C77.7 426.6 77.7 439.6 85.7 447.6L93.8 455.7C143.5 505.4 213.7 522.2 277.2 506C296.6 486.7 311.2 463.1 319.6 437.2C299.7 444.2 278.3 448 256.1 448C220.4 448 186.9 438.2 158.3 421.2zM421.3 481.8C421.2 494.8 419.7 507.9 416.6 520.8L413.2 535C411.5 541.9 413.6 549.2 418.7 554.3C426.7 562.3 439.7 562.3 447.7 554.3L455.8 546.2C505.5 496.5 522.3 426.3 506.1 362.8C486.8 343.4 463.2 328.8 437.3 320.4C444.3 340.3 448.1 361.7 448.1 383.9C448.1 419.6 438.3 453.1 421.3 481.7zM481.9 218.7C494.9 218.8 508 220.3 520.9 223.4L535.1 226.8C542 228.5 549.3 226.4 554.4 221.3C562.4 213.3 562.4 200.3 554.4 192.3L546.3 184.2C496.6 134.5 426.4 117.7 362.9 133.9C343.5 153.2 328.9 176.8 320.5 202.7C340.4 195.7 361.8 191.9 384 191.9C419.7 191.9 453.2 201.7 481.8 218.7z';

const GALAXY_VIEWBOX = 640;
const GALAXY_MARK_SIZE = 54;
const GALAXY_MARK_X = 400;
const GALAXY_MARK_Y = 60.98 - GALAXY_MARK_SIZE * 0.92;
interface GalaxyMarkProps {
  faintColor: string;
  solidColor: string;
}
const GalaxyMark: FC<GalaxyMarkProps> = ({ faintColor = '#FB5CF6', solidColor = '#38bdf8' }) => (
  <g
    transform={`translate(${GALAXY_MARK_X}, ${GALAXY_MARK_Y}) scale(${GALAXY_MARK_SIZE / GALAXY_VIEWBOX})`}
    aria-hidden
  >
    <path opacity=".4" fill={faintColor} d={GALAXY_FAINT_PATH} />
    <path fill={solidColor} d={GALAXY_SOLID_PATH} />
  </g>
);

export const Warp12Logo: FC<Warp12LogoProps> = ({
  width,
  className,
  marginLeft = '-12px',
  warpColor = '#38bdf8',
  numberColor = '#ffffff',
  taglineColor = '#e2e8f0',
  taglineOff,
  factor,
}) => {
  const factorXTranslate = factor !== undefined && factor >= 10 ? 383.22 : 400;

  return (
    <svg
      style={{ marginLeft }}
      id="Layer_1"
      data-name="Layer 1"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 531.41 111.65"
      width={width}
      className={className}
      aria-label="Warp 12"
    >
      <defs>
        <style>{`
          .cls-1 {
            font-family: 'Federation', Federation, sans-serif;
            font-size: 72px;
            fill: ${numberColor};
          }

          .cls-2 {
            font-family: 'FederationWide', FederationWide, sans-serif;
            font-size: 72px;
            fill: ${warpColor};
          }

          .cls-3 {
            font-family: 'Nova Light', Federation, sans-serif;
            font-size: 42px;
            fill: ${taglineColor};
          }
        `}</style>
      </defs>
      <text className="cls-2" transform="translate(17.16 60.98)">
        <tspan x="0" y="0">
          Warp
        </tspan>
      </text>
      {factor !== undefined ? (
        <text className="cls-1" transform={`translate(${factorXTranslate} 60.98)`}>
          <tspan x="0" y="0">
            {factor}
          </tspan>
        </text>
      ) : (
        <GalaxyMark faintColor='#FB5CF6' solidColor='#38bdf8' />
      )}
      {!taglineOff && (
      <text className="cls-3" transform="translate(345.15 97.29)">
        <tspan x="0" y="0">
          Interstellar Dominoes
        </tspan>
      </text>
      )}
    </svg>
  );
};
