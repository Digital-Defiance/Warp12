...
//! Rather than pull in a third-party crate, we bind a one-shot localhost server
//! on an ephemeral port with the standard library, capture the single OAuth
//! redirect, reply with a friendly "you can close this window" page, and hand
//! the captured callback URL back to the webview.
//!
//! Delivery uses a **channel + command return value** rather than a Tauri event:
//! `start_oauth_server` binds the socket, returns the port, and spawns a thread
//! that pushes the captured URL into a channel; `await_oauth_redirect` then
//! resolves with that URL. Because the channel buffers the value, it does not
//! matter whether the redirect arrives before or after the frontend starts
//! awaiting — there is no lost-event race.
...
/// The first request line looks like `GET /path?query HTTP/1.1`; pull out the
...
/// Bind a loopback server on a random free port and return that port. A
/// background thread waits for the OAuth redirect and pushes the full callback
/// URL into a channel retrieved by [`await_oauth_redirect`].
#[tauri::command]
pub async fn start_oauth_server(servers: State<'_, OAuthServers>) -> Result<u16, String> {
    ...
    std::thread::spawn(move || {
        for connection in listener.incoming() {
            ...
            // Ignore favicon/preconnect probes; only the OAuth redirect carries
...
/// Await the captured redirect URL for a server started by
/// [`start_oauth_server`]. Blocks (off the async workers) until the redirect
/// arrives or a 5-minute timeout elapses.
#[tauri::command]
