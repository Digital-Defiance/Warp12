...
\item
  ...
  the inverted tally in Section~VI (negate hands; going out is a large
  positive dump). The campaign still ends on lowest cumulative total.
\item
...
Section~IV (pip totals, double-blank setting, Salamander if on). Then
write the \textbf{campaign adjustment} as follows --- do \textbf{not}
...
\tightlist
\item
  \textbf{Who emptied their hand} (went out) takes a large positive
  dump --- you wanted to \emph{keep} tiles, not empty. For Warp~$N$,
  use \(2N\times 13\) (Warp~12 \(\rightarrow\) \textbf{312}; Warp~18
  \(\rightarrow\) \textbf{468}). Add that amount to their campaign
  total.
\item
  \textbf{Everyone else} takes the \textbf{negation} of their hand total:
  subtract the hand from the campaign (a hand of 40 becomes
  \textbf{$-40$}). Holding more pips improves your standing; a thin hand
  hurts.
\item
  ...
  that hand term, same as the app. Longest Trail still cannot push a
  \emph{normal} (odd-round) total below 0; on inverted rounds the ledger
  is already allowed to go negative by design.
\item
...
\textbf{Why the signs look backwards:} On an even round, a fat hand must
\emph{help} you under a lowest-total campaign. Negating the hand does
that on paper: \texttt{$-40$} beats \texttt{$-10$}, and both beat a
go-out's \texttt{$+312$}. Offline, just keep a running total column and
add / subtract the even-round adjustment as written; you do not need a
...
Armstrong holds 40 pips; Lovell holds 12.

\begin{itemize}
\tightlist
\item
  Earhart (went out): \textbf{$+312$} $\rightarrow$ campaign
  \(30+312=\textbf{342}\).
\item
  Armstrong (40 in hand): \textbf{$-40$} $\rightarrow$ campaign
  \(20-40=\textbf{$-20$}\).
\item
  Lovell (12 in hand): \textbf{$-12$} $\rightarrow$ campaign
  \(25-12=\textbf{13}\).
\end{itemize}
...
\textbf{Tactical Impact:} Going out in even rounds is catastrophic.
Medium hands become optimal. AI struggles to adapt. Lower-skilled
players win \textasciitilde65\% of the time (35.6\% higher-skill win
rate in calibration --- intentionally inverts skill ordering).
