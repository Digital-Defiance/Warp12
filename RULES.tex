...
the deal, personal trails, community / Neutral Zone trail, distress
beacons, doubles, boneyard, multi-round scoring). \textbf{Section VI}
...
\subsubsection{Modules by
objective}\label{modules-by-objective}

Optional Modules (Alpha through Lambda) are written
\textbf{twice} below --- once for each sector objective. Read
\textbf{only} the block that matches how you launched:
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Points campaign} & \textbf{Points campaign --- Modules}
(immediately below). Stop when you reach the Go-out block. \\
\textbf{Go out} & Skip the Points campaign Module text. Use
\textbf{Go-out --- Modules} (after Zeta in the Points block). \\
\end{longtable}
...
\paragraph{Points campaign ---
Modules}\label{points-campaign--modules}

The Module text in this block applies when the sector objective is
\textbf{Points campaign}. Do \textbf{not} use it for Go-out.
...
\textbf{Peek Uncharted Sector} & The invoker sees the next coordinate that
would be drawn from Uncharted Sectors (hidden from others). \\
...
the highest double (\texttt{maxPip-maxPip}) at round end, the full
Salamander penalty (double its pips) lands on the
\textbf{highest-points captain among those who did not win the round}
instead --- the holder pays nothing for that coordinate.
If the holder \textbf{is already} that captain, the swap does
\textbf{not} transfer: they keep the full doubled penalty (same as
Salamander without swap). \\
\textbf{All Stop! echo} & Any captain going out this round must call All
Stop! before the sector closes. \\
\textbf{Continuum Wager} & Draw two coordinates from Uncharted
...
If a round ends and a captain holds the \textbf{highest double for the
Warp Factor} (\texttt{maxPip-maxPip}) in hand, that coordinate scores
\textbf{double its both-ends pip value}. Round 1 never applies (that
double is Spacedock). From round 2 onward it is in circulation.

Examples: Warp 12 \textbf{12-12 → 48} (base 24); Warp 9 \textbf{9-9 →
36}; Warp 15 \textbf{15-15 → 60}; Warp 18 \textbf{18-18 → 72}.

With \textbf{Salamander swap} (Continuum, Module Alpha), the
\textbf{entire doubled penalty} transfers to the
\textbf{highest-points captain among non-winners} of the round --- the
highest-double holder pays \textbf{nothing} for that coordinate, and that
leader receives the full amount. The round winner is not eligible as the
swap target (they score 0 for the round hand). If the holder is already
that campaign leader, there is no transfer: they keep the full doubled
penalty, exactly as under Salamander without Continuum swap.
...
\item
  ...
  coordinate.
\item
  \textbf{Replenish:} Whenever a coordinate leaves the Sensor Grid
  (Sensor Sweep, or any module that draws through the market), replace it
  from Uncharted Sectors so the market returns to its configured size
  while Uncharted still has coordinates. Blind Jumps draw only from
  Uncharted and do not change the face-up market.
\item
...
\textbf{Warp Drive Engagement (Spool):} On your turn (when not blocked by
Red Alert or Subspace Fracture), you may \textbf{engage warp drive} to
draw coordinates from \textbf{Uncharted Sectors only}---draw one at a
time until you draw a coordinate that cannot be charted to your chosen
route, or Uncharted is empty. Each matching coordinate is charted and
the route endpoint updates. The first mismatch goes to your hand and the
spool \textbf{stops}; undrawn coordinates remain in Uncharted (you never
draw past the stop).

\textbf{Doubles during spool:} If a matching double is drawn, the next
draw must cover it (or, when Subspace Fracture applies to that double,
three matching stabilizers must follow). If the cover or any required
stabilizer fails to match --- or Uncharted runs out before the double is
satisfied --- the unfinished double is \textbf{retrieved to your hand}
along with any failed cover or stabilizer draw(s). The route endpoint
reverts to its value before that double. \textbf{No Red Alert} and
\textbf{no Subspace Fracture} remain on the table from that aborted
double.

Spooling does \textbf{not} take coordinates from the Sensor Grid (Module
Gamma); sensor takes remain a separate action. Spooling is part of Module
Delta; Module Theta does not unlock it. Engage Warp Drive is unavailable
when Uncharted Sectors are empty.
...
receive the bonus. The bonus \textbf{cannot reduce a captain's round
score below 0} (a go-out or tiny hand stays at 0 after all normal-round
adjustments). Engaging Warp Drive / Spool is \textbf{not} part of
Theta---it ships with Module Delta.

\textbf{Tactical Impact:} Creates opposing incentives with Module Delta
(hot potato and NZ spool push toward Neutral Zone risk; longest trail
pulls toward your own trail).
...
trail), the \textbf{next captain} draws \textbf{2 coordinates} from Uncharted
...
\item
  \textbf{Rewards playing from hand} --- skilled coordinate management
  (holding versatile coordinates, planning ahead) gains an edge
\item
...
\textbf{campaign still ranks by lowest cumulative total}. Kappa does not
replace that ledger --- it only changes how each even round is written
into it.

\textbf{Which rounds invert:}

\begin{itemize}
\tightlist
\item
  \textbf{Odd rounds} (1, 3, 5\ldots): use the normal tally in
  Section~IV (empty hand = 0; others add hand pips; lower is better).
\item
  \textbf{Even rounds} (2, 4, 6\ldots): use the inverted tally below.
\end{itemize}

\textbf{Even-round tally (tabletop):} Count hands the same way as
Section~IV (pip totals, double-blank setting, Salamander if on). Find
the \textbf{top hand} --- the largest hand pip total any captain still
holds this round. Then write each captain's \textbf{campaign
adjustment} as a \emph{spread} off that top hand --- do \textbf{not}
award ``0 for going out'' on these rounds.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{Baseline = top hand.} Every captain starts the round at that
  number.
\item
  \textbf{Subtract your own hand.} The captain holding the top hand
  nets \textbf{0} --- they win the round. A thinner hand nets the
  difference (top hand minus theirs). Whoever \textbf{emptied their
  hand} (went out) kept nothing, so they take the \textbf{full baseline}
  --- the worst outcome, since on an inverted round you wanted to
  \emph{keep} coordinates, not shed them.
\item
  Apply any other enabled Module add-ons from this section (Hazard pass
  penalties, Longest Trail $-3$, Temporal Debt, and so on) \textbf{after}
  that hand term, same as the app.
\item
  The sector winner at the end is still whoever has the
  \textbf{lowest cumulative} score --- Kappa never switches the campaign
  to ``highest wins.''
\end{enumerate}

\textbf{Scoring floor and ties (both round types):} The lowest score a
captain can \emph{show} is \textbf{zero}. The only term that can dip a
round below zero is the Longest Trail bonus ($-3$); that sub-zero amount
is \textbf{kept} so it can break an otherwise-tied campaign (a true
$-3$ edges a $0$), but it is never \emph{displayed} as a negative ---
scoreboards clamp the shown total to zero and use the raw value only to
rank level totals.

\textbf{Why the top hand wins:} On an even round a fat hand should
\emph{help} you under a lowest-total campaign, and the spread does
exactly that: the biggest hand contributes 0, thinner hands contribute
the gap, and an empty hand (a go-out) contributes the most. Unlike the
old fixed dump, the swing scales with the actual hands --- one bad even
round no longer dwarfs an entire campaign. Offline, just keep a running
total column and add each captain's spread as written.

\textbf{Worked example (Warp 12, even round, three captains).} Before
the round: Armstrong 20, Lovell 25, Earhart 30. Earhart goes out;
Armstrong holds 40 pips; Lovell holds 12. The \textbf{top hand is 40}
(Armstrong).

\begin{itemize}
\tightlist
\item
  Armstrong (40 in hand): \(40-40=\) \textbf{$+0$} $\rightarrow$
  campaign \(20+0=\textbf{20}\) --- wins the round with the fattest
  hand.
\item
  Lovell (12 in hand): \(40-12=\) \textbf{$+28$} $\rightarrow$ campaign
  \(25+28=\textbf{53}\).
\item
  Earhart (went out): \(40-0=\) \textbf{$+40$} $\rightarrow$ campaign
  \(30+40=\textbf{70}\).
\end{itemize}

Armstrong leads the campaign for holding the most; Earhart is penalized
for emptying out when coordinates were the prize --- but by a proportional 40,
not a flat 312.

\textbf{Tactical Impact:} Going out in even rounds is the worst outcome
(you take the full top-hand baseline). Medium-to-large hands become
optimal. AI struggles to adapt. Lower-skilled players win
\textasciitilde65\% of the time (35.6\% higher-skill win rate in
calibration --- intentionally inverts skill ordering).

\textbf{Status:} Exhibition/Warped mode only---\textbf{intentionally
breaks skill ordering} for casual chaos play. Never rated (TEI does not
use these scores).
...
\textbf{Tabletop (physical coordinates):} Do \textbf{not} slide entire
warp trails when a Wormhole opens. Keep the coordinates where
they sit on the hub and \textbf{re-seat} the captain who opened the
Wormhole so they sit in front of the arm that is now their private
trail (the former Neutral Zone arm). Mark the arm they left as the new
\textbf{Neutral Zone} (public). Other captains stay put. Digital play
swaps ownership labels instead; the board geometry is the same either
way.
...
\paragraph{Go-out --- Modules}\label{go-out--modules}

The Module text in this block applies when the sector objective is
\textbf{Go out}. Campaign points are never tallied. Do \textbf{not}
mix these rules with the Points campaign Module text above --- each
Module below is complete for Go-out on its own.

\subsubsection{Module Alpha --- The Continuum
\emph{(Go-out)}}\label{module-alpha--the-continuum-go-out}

When \textbf{enabled}, charting \textbf{0-0 on your own Warp Trail}
triggers a \textbf{Continuum Flash} before helm passes. That captain
immediately chooses \textbf{one} directive for the rest of the sector
(cleared when the sector ends):
...
\textbf{Reverse turn order} & Helm passes counter-clockwise for the rest
of the sector. \\
\textbf{Skip Lightest Hand} & The captain(s) holding the fewest
coordinates skip their next turn. If several captains tie for fewest,
all of them skip (including the invoker if they are among the
lightest). \\
\textbf{Peek Uncharted Sector} & The invoker sees the next coordinate that
would be drawn from Uncharted Sectors (hidden from others). \\
\textbf{Temporal inversion} & Turn order reverses until the next double
...
\textbf{Fracture immunity} & The next double on an own trail will not
open Subspace Fracture (Red Alert still applies). \\
\textbf{Force Draw} & The invoker chooses one opposing captain; that
captain immediately draws \textbf{1} coordinate from Uncharted Sectors
(or the Sensor Grid if Uncharted is empty and Module Gamma is on). If
neither pool has a coordinate, the draw is canceled. \\
\textbf{All Stop! echo} & Any captain going out this sector must call All
Stop! before the sector closes. \\
\textbf{Continuum Wager} & Draw two coordinates from Uncharted
Sectors --- keep one, return the other face-down. \\
\end{longtable}
...
\subsubsection{Module Beta --- Salamander Surge
\emph{(Go-out)}}\label{module-beta--salamander-surge-go-out}

When \textbf{enabled}, charting the \textbf{highest double for the Warp
Factor} (\texttt{maxPip-maxPip}) from hand --- \textbf{not} as Spacedock
/ the opening engine play --- triggers a \textbf{Salamander Surge}:
every opposing captain immediately draws \textbf{1} coordinate from
Uncharted Sectors (or the Sensor Grid if Uncharted is empty and Module
Gamma is on).
...
Examples of the Salamander double: Warp 12 \textbf{12-12}; Warp 9
\textbf{9-9}; Warp 15 \textbf{15-15}; Warp 18 \textbf{18-18}.

\subsubsection{Module Gamma --- Long-Range Sensor Sweep
\emph{(Go-out)}}\label{module-gamma--long-range-sensor-sweep-go-out}
...
\subsubsection{Module Delta --- Hot Potato (Neutral Zone Hazard)
\emph{(Go-out)}}\label{module-delta-hot-potato-go-out}

The Neutral Zone is highly unstable subspace. Playing there is
convenient but dangerous---every contact transfers custody of a volatile
\textbf{Hazard Marker}, and captains stuck holding it when they pass
suffer immediate hand-bloat penalties.
...
\textbf{Pass Penalty:} If you \textbf{pass} your turn while holding the
Hazard Marker (cannot chart, drew or pile is empty, beacon already
deployed), draw \textbf{2} coordinates from Uncharted Sectors (or the
Sensor Grid if Uncharted is empty and Module Gamma is on). If both pools
are completely empty, skip your \textbf{next} turn instead.
...
\textbf{Sector Initialization:} The sector starter receives the Hazard
Marker at the beginning of the sector (they got the ``privilege'' of the
opening coordinate).
...
\textbf{Tactical Impact:} The Neutral Zone becomes a strategic dumping
ground with real risk. Captains must weigh convenience against the
danger of getting stuck with the marker and being forced to pass into
extra draws. Spooling stretches a trail (or the Neutral Zone) at
hand-bloat risk.

\subsubsection{Module Theta --- Trail Momentum
\emph{(Go-out)}}\label{module-theta--trail-momentum-go-out}

Strategic long-range navigation rewards extended personal trails.

\textbf{Trail Momentum:} The first captain whose \textbf{personal} Warp
Trail reaches \textbf{5} coordinates (trail length $\geq 5$ after a
chart, including a spool that extends the trail) earns one immediate
\textbf{extra turn}. Claimable \textbf{once per round}. Engaging Warp
Drive / Spool is \textbf{not} part of Theta---it ships with Module
Delta.

\textbf{Tactical Impact:} Creates opposing incentives with Module Delta
(hot potato and NZ spool push toward Neutral Zone risk; trail momentum
pulls toward your own trail).

\subsubsection{Module Iota --- Double Down
\emph{(Go-out)}}\label{module-iota-double-down-go-out}
...
\subsubsection{Module Eta --- Desperation Dig
\emph{(Go-out)}}\label{module-eta--desperation-dig-go-out}

Drawing from the unknown can be a last resort. When \textbf{enabled},
Temporal Debt is an optional push-your-luck dig rather than a scoring
penalty.

\textbf{Mechanic (Desperation Dig):}

\begin{itemize}
\tightlist
\item
  When you cannot chart and must draw from \textbf{Uncharted Sectors},
  you may invoke Temporal Debt.
\item
  Draw up to \textbf{3} coordinates one at a time, stopping immediately
  when you draw a playable coordinate --- \textbf{chart that
  coordinate} as part of this action (then finish any Red Alert /
  Fracture duties as usual).
\item
  Your Distress Beacon stays \textbf{open for your next two turns}
  (opponents may chart onto your trail while it is open).
\item
  ...
  full.
\item
  \textbf{Sensor Sweep takes (Module Gamma)} are not a Desperation Dig
  and do not incur the beacon cost.
\end{itemize}

\textbf{Tactical Impact:} Lets a blocked captain gamble for a line out of
a dead hand at the cost of an open trail. Rewards knowing when the dig is
worth the exposure.

\subsubsection{Module Kappa --- Hand Exchange
\emph{(Go-out; Warped --- exhibition
only)}}\label{module-kappa--hand-exchange-go-out}

Reality bends once per round. When \textbf{enabled}, the first time any
captain charts a \textbf{double} other than Spacedock, the captain with
the \textbf{most} coordinates in hand and the captain with the
\textbf{fewest} resolve an exchange:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  The larger hand randomly takes \textbf{1} coordinate from the smaller
  hand.
\item
  The larger hand then chooses \textbf{1} coordinate from their own hand
  to give back.
\end{enumerate}
...
\textbf{Status:} Exhibition/Warped mode only --- casual chaos play. Never
rated.

\subsubsection{Module Lambda --- Wormholes
\emph{(Go-out; Warped --- exhibition
only)}}\label{module-lambda-wormholes-go-out}
...
\subsubsection{Module Epsilon --- Tactical Requisition (Draft)
\emph{(Go-out)}}\label{module-epsilon--draft-go-out}

\textbf{Unavailable.} Go-out sectors must leave drafting \textbf{off}.
The race depends on captains adapting to the inefficiencies of their
dealt hands; sculpting a starting loadout removes that friction.

\subsubsection{Module Zeta --- Fleet Squadrons (Warp Crews)
\emph{(Go-out)}}\label{module-zeta--fleet-squadrons-go-out}
...
\item
  ...
  board.
\item
  \textbf{Victory:} A squadron scores a round win when \emph{one} captain
  clears their hand (no pip tally). Sector victory follows the go-out
  structure chosen at launch.
\end{itemize}
...
\item
  How many coordinates each opponent is \textbf{holding} (hand count only ---
  not which coordinates).
\item
  How many coordinates remain in \textbf{Uncharted Sectors}.
\end{itemize}

An AI officer \textbf{cannot} see which specific coordinates are in an
opponent\textquotesingle s hand. It does not read hidden coordinates from the
...
\item
  ...
  assignment from the pool of coordinates not on the table and not in the
...
\textbf{\href{https://iwgf.org}{iwgf.org}} when the client is signed in
to Firebase. Team campaigns, unrated lobby modes, and builds without a
working stats backend do not update the public boards.

\begin{quote}
\textbf{In-person play:} While we encourage Warp to be played in person with physical dominoes --- around a table, with friends and family --- the TEI rating system requires digital tracking to compute and maintain ratings accurately. For tabletop games, use the \href{https://iwgf.org/calculator}{TEI calculator} or officiated match codes to report results and update your rating. The full online app (\href{https://warp.iwgf.org}{warp.iwgf.org}) and the leaderboards (\href{https://iwgf.org}{iwgf.org}) handle all TEI computation automatically.
\end{quote}
...
\item
  ...
  \href{https://iwgf.org/crews}{iwgf.org/crews}; scoped \textbf{group
...
\item
  ...
  \href{https://iwgf.org/crews}{iwgf.org/crews}. \textbf{Google sign-in
...
The \href{https://iwgf.org/calculator}{TEI calculator} can preview
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Go-out TEI} & Sector win under the chosen go-out structure
(sudden death, fixed rounds, or first to $X$) \\
...
\textbf{Placement, not margin.} Ratings track \emph{where} you finish,
not \emph{by how much}. When a match is scored, the final standings are
collapsed to finishing places (1st, 2nd, 3rd\ldots) and your rating
moves on those places alone: every captain you finish ahead of nudges
you up, every one who finishes ahead of you nudges you down, each
weighted by how strong that opponent was. The size of the point gap
never enters the math --- losing a points campaign by 3 and losing it by
300 move your TEI by \textbf{exactly the same amount}, and a blowout win
counts no more than a one-point squeaker. (Tie on the cumulative total
and you split the place, a half-step each.) So a single ugly round ---
say a Module~Kappa inversion or a fat go-out penalty that dumps a big
number on you --- only costs rating if it actually drops you \emph{past}
another captain; if you still finish in the same place, your TEI is
unchanged. Chase \textbf{finishing order}, not a pretty scoreline.
...
\begin{longtable}[]{@{}>{\raggedright\arraybackslash}p{0.34\linewidth}>{\raggedright\arraybackslash}p{0.13\linewidth}>{\raggedright\arraybackslash}p{0.47\linewidth}@{}}
\toprule\noalign{}
...
(1-coordinate catch), and All Stop! ceremony on standard \textbf{Sections
I--V} gameplay.

\begin{longtable}[]{@{}>{\raggedright\arraybackslash}p{0.30\linewidth}>{\raggedright\arraybackslash}p{0.30\linewidth}>{\raggedright\arraybackslash}p{0.34\linewidth}@{}}
\toprule\noalign{}
...
Voluntary beacon while able to play & \textbf{No} & \textbf{No} (unless
\textbf{Manual shield control} --- own trail required) \\
Play elsewhere while marked & Allowed; beacon stays & Same \\
Play on own trail while marked & Beacon \textbf{must} come off & Shields
Up --- same \\
...
Highest-double Salamander Penalty & --- & Salamander --- Official Warp
preset \\
NZ win announcement & --- & All Stop! ceremony --- Official Warp
preset \\
One coordinate left announce & --- & Drop to Impulse --- Official Warp preset
(1-coordinate catch) \\
...
Solo TEI vs AI & --- & Section VIII --- iwgf.org; unassisted matches
only \\
