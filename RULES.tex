...
multi-trail \textbf{knock}. (No, reaching one tile does not let you
...
instead --- the holder pays nothing for that tile.
...
Warp Factor} (\texttt{maxPip-maxPip}) in hand, that tile scores
...
highest-double holder pays \textbf{nothing} for that tile, and that
...
draw coordinates from Uncharted Sectors---draw tiles one at a time until
you draw a tile that cannot be charted to your chosen route. All
matching tiles are charted; the mismatch (and any remaining tiles) go to
