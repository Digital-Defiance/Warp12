...
describe('AI rating calibration (self-play)', () => {
  it('reference ratings are spaced 200 points apart (points) and 250 (go-out)', () => {
    expect(REFERENCE_AI_RATING.lieutenant - REFERENCE_AI_RATING.ensign).toBe(
      200
    );
    expect(REFERENCE_AI_RATING.commander - REFERENCE_AI_RATING.lieutenant).toBe(
      200
    );
    expect(
      GO_OUT_REFERENCE_AI_RATING.lieutenant - GO_OUT_REFERENCE_AI_RATING.ensign
    ).toBe(250);
    expect(
      GO_OUT_REFERENCE_AI_RATING.commander - GO_OUT_REFERENCE_AI_RATING.lieutenant
    ).toBe(250);
    expect(referenceRatingForObjective('points')).toBe(REFERENCE_AI_RATING);
    expect(referenceRatingForObjective('go-out')).toBe(GO_OUT_REFERENCE_AI_RATING);
  });

  it('implied rating gap matches a 76% win rate near 200 points', () => {
    expect(impliedRatingGap(0.76)).toBeGreaterThan(170);
    expect(impliedRatingGap(0.76)).toBeLessThan(230);
  });
  ...
  describe('points objective', () => {
    ...
    it('observed win rates align with fixed opponent rating spacing', () => {
      const matrix = runCalibrationMatrix({
        games: CALIBRATION_GAMES,
        objective: 'points',
        seed: CALIBRATION_SEED,
  ...
  it(
    'completes games with Drop to Impulse house rule enabled',
    () => {
