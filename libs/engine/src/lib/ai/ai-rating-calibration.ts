...
/** Mirrors apps/Warp12 stats-rating AI_OPPONENT_RATING — keep in sync when tuning. */
export const REFERENCE_AI_RATING: Record<WarpSkillLevel, number> = {
  ensign: 1000,
  lieutenant: 1200,
  commander: 1400,
};
...
export const GO_OUT_REFERENCE_AI_RATING: Record<WarpSkillLevel, number> = {
  ensign: 1000,
  lieutenant: 1250,
  commander: 1500,
};

export function referenceRatingForObjective(
  objective: GameObjective
): Record<WarpSkillLevel, number> {
  return objective === 'go-out' ? GO_OUT_REFERENCE_AI_RATING : REFERENCE_AI_RATING;
}

/**
 * Calibration roadmap (see repo todos: readme-rating, paper-rating, rating-per-players):
 * - README: Rating tracks, advisor exclusion, how self-play validates tier ratings
 * - Per player-count (2–8): tune presets so implied Δ rating matches targets at each table size
 * - Optional write-up on rating-feedback-driven AI skill tiers
 */
...
export interface SkillMatchupResult {
  ...
  /** Implied TEI gap from observed higher-skill win rate. */
  impliedRatingGap: number | null;
  /** Expected higher-skill win rate from REFERENCE_AI_RATING spacing. */
...
export function impliedRatingGap(higherSkillWinRate: number): number {
...
function skillRank(skill: WarpSkillLevel, objective: GameObjective): number {
  return referenceRatingForObjective(objective)[skill];
}
...
export function runSkillMatchup(
  ...
  if (symmetric && match.completed > 0) {
    ...
    impliedGap = impliedRatingGap(higherSkillWinRate);
    const reference = referenceRatingForObjective(options.objective);
...
export function formatMatchupResult(result: SkillMatchupResult): string {
  ...
  const expectedHigher =
    result.expectedHigherSkillWinRate !== null
      ...
      : '—';
  const gap =
    result.impliedRatingGap !== null && Number.isFinite(result.impliedRatingGap)
      ? Math.round(result.impliedRatingGap)
      : '—';

  return `${label}: ${higher} wins ${wins}/${result.completed} (${rate}%, implied ΔTEI ${gap}, expected ${expectedHigher}%)`;
}
