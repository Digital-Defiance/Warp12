...
describe('Q-Continuum', () => {
  it('requires a Q-Flash after charting 0-0', () => {
    ...
    if (chart.ok) {
      expect(chart.state.round?.qPendingInvoker).toBe('a');
  ...
  it('applies reverse turn order for the rest of the round', () => {
    ...
    state = flash.state;
    expect(state.modules.qContinuum.activeFlash?.effect.kind).toBe(
      'reverse-turn-order'
    );
    expect(state.round?.qEffects?.reverseTurnOrder).toBe(true);
    expect(state.round?.activePlayerId).toBe('c');
  });

  it('clears active Q-Flash when the round scores', () => {
    ...
    if (result.ok) {
      expect(result.state.modules.qContinuum.activeFlash).toBeNull();
      expect(result.state.round?.qEffects).toBeNull();
    }
  });
});
