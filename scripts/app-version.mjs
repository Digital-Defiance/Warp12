#!/usr/bin/env node
/**
 * Canonical Warp 12 app version: semver 0.MINOR.BUILD where BUILD is also
 * iOS bundleVersion and Android versionCode (kept in sync).
 *
 *   node scripts/app-version.mjs print
 *   node scripts/app-version.mjs set 0.7.51
...
const PKG_JSON = path.join(ROOT, 'apps/Warp12/package.json');
const TAURI_CONF = path.join(ROOT, 'apps/Warp12/src-tauri/tauri.conf.json');
const CARGO_TOML = path.join(ROOT, 'apps/Warp12/src-tauri/Cargo.toml');
...
export function parseAppVersion(version) {
  ...
  if (!match) {
    throw new Error(
      `expected 0.MINOR.BUILD semver (e.g. 0.6.51), got ${version}`
    );
  }
  return {
    version: normalized,
