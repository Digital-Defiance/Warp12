...
MANIFEST="${WARP12_ROOT}/apps/Warp12/src-tauri/gen/android/app/src/main/AndroidManifest.xml"
TAURI_CONF="${WARP12_ROOT}/apps/Warp12/src-tauri/tauri.conf.json"
TAURI_LOCAL="${WARP12_ROOT}/apps/Warp12/src-tauri/tauri.conf.local.json"
ROOT_ENV="${WARP12_ROOT}/.env"
APP_ENV="${WARP12_ROOT}/apps/Warp12/.env"
...
GRADLE_KTS="${WARP12_ROOT}/apps/Warp12/src-tauri/gen/android/app/build.gradle.kts"
...
if [ -n "${APPLE_BUNDLE_ID:-}" ]; then
  node "${WARP12_ROOT}/scripts/tauri-config-from-env.mjs" --write "$TAURI_LOCAL" >/dev/null
fi

# Tauri emits Kotlin under APPLE_BUNDLE_ID, but gen/android often still has
# com.example.warp12 in build.gradle.kts — that makes BuildConfig resolve to the
...
if scheme:
    ...
    print(
        ...
        "in .env / apps/Warp12/.env or the environment",
        file=sys.stderr,
    )
