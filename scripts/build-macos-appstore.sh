...
Usage: bash scripts/build-macos-appstore.sh [OPTIONS] [VERSION] [-- extra tauri build args...]
  ...
  VERSION          Semver (default: apps/Warp12/package.json "version").
...
Output:
  apps/Warp12/src-tauri/target/universal-apple-darwin/release/bundle/macos/<productName>.app
  apps/Warp12/src-tauri/target/universal-apple-darwin/release/bundle/pkg/<productName>_<version>.pkg
EOF
  exit 1
}
...
Download from developer.apple.com → Profiles (Mac App Store) and save as:
  ${TAURI_DIR}/Warp_12.provisionprofile
