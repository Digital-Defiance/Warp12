...
Environment:
  ...
  ANDROID_KEYSTORE                  Default apps/Warp12/src-tauri/Warp_12_upload.jks
  ANDROID_KEY_ALIAS                 Default upload
  ANDROID_KEYSTORE_PASSWORD         Keystore password (prompted if unset)

Output (typical):
  apps/Warp12/src-tauri/gen/android/app/build/outputs/bundle/universalRelease/*.aab
EOF
  exit 1
}
...
verify_keystore_password() {
  ...
  local err_file
  err_file="$(mktemp -t warp12-keystore-err.XXXXXX)"
...
[ -d "$ANDROID_DIR" ] || die "missing ${ANDROID_DIR} — run: cd apps/Warp12 && ../../node_modules/.bin/tauri android init"
