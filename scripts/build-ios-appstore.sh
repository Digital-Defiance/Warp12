...
Required local files (gitignored):
  apps/Warp12/src-tauri/Warp_12_iOS.mobileprovision   iOS App Store profile
  apps/Warp12/src-tauri/Warp_12_iOS.p12               Apple Distribution cert
...
prepare_signing_keychain() {
  ...
  SIGNING_KEYCHAIN_DIR="$(mktemp -d -t warp12-sign.XXXXXX)" || die "failed to create temp directory for signing keychain"
...
verify_certificate_password() {
  ...
  err_file="$(mktemp -t warp12-verify-err.XXXXXX)"
  kc_dir="$(mktemp -d -t warp12-verify.XXXXXX)" || return 1
...
write_export_options_plist "${PROFILE_NAME:-Warp 12 App Store}" "$BUNDLE_ID"
assert_manual_signing_env
