#!/usr/bin/env node
/**
 * Generate apps/Warp12/src-tauri/tauri.conf.local.json from environment.
 *
 * Precedence: process.env already set by scripts/lib/warp-env.sh (which loaded
 * .env without overwriting exports). This helper only reads process.env.
 *
 * Usage:
...
function requireEnv(name) {
  ...
  if (!value) {
    console.error(
      `error: ${name} is required to generate tauri.conf.local.json (see .env.example)`,
    );
    process.exit(1);
  }
  return value;
...
function buildOverride() {
  // Load root then app env if caller did not already export vars
  loadDotEnvIfNeeded(join(ROOT, '.env'));
  loadDotEnvIfNeeded(join(ROOT, '.env.local'));
  loadDotEnvIfNeeded(join(ROOT, 'apps/Warp12/.env'));
  loadDotEnvIfNeeded(join(ROOT, 'apps/Warp12/.env.local'));
  ...
  // Deep-link schemes from Google OAuth client IDs (Android/desktop preferred)
  const androidScheme =
  ...
  // Drop undefined nested keys for cleaner JSON
...
function main() {
  ...
  if (mode === '--write') {
    const out = resolve(
      args[1] || join(ROOT, 'apps/Warp12/src-tauri/tauri.conf.local.json'),
    );
  ...
  console.error('Usage: node scripts/tauri-config-from-env.mjs --print|--write [path]');
  process.exit(1);
}
